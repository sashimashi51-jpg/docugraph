...
CALL :print_highlight "Updating Docugraph to latest"
CALL :update_latest
IF ERRORLEVEL 1 GOTO :end
...
:update_latest
FOR /F "tokens=1,2" %%a in ('pip list') do if "%%a"=="docugraph-app" set current_version=%%b
...
IF EXIST "pyproject.toml" (
    ...
    python -m pip install git+https://github.com/Cinnamon/docugraph.git@"%app_version%"#subdirectory=libs/docugraph
    python -m pip install git+https://github.com/Cinnamon/docugraph.git@"%app_version%"#subdirectory=libs/ktem
    python -m pip install --no-deps git+https://github.com/Cinnamon/docugraph.git@"%app_version%"
) || (
...
FOR /F "tokens=1,2" %%a in ('pip list') do if "%%a"=="docugraph-app" set updated_version=%%b
