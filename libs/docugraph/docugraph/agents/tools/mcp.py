"""MCP Tool for docugraph agents.

Bridges the MCP SDK's tool schema with docugraph's BaseTool abstraction
...
# ---------------------------------------------------------------------------
# JSON Schema â†’ Pydantic helpers
# ---------------------------------------------------------------------------
...
def format_tool_list(
    ...
    """
    lines = [f"âœ… Connected! Found <b>{len(tool_infos)}</b> tool(s):<br>"]
    for t in tool_infos:
        desc = (t.get("description") or "No description")[:120]
        if enabled_tools is not None:
            check = "âœ…" if t["name"] in enabled_tools else "â¬œ"
            lines.append(f"&nbsp;&nbsp;{check} <b>{t['name']}</b> â€” {desc}<br>")
        else:
            lines.append(f"&nbsp;&nbsp;â€¢ <b>{t['name']}</b> â€” {desc}<br>")
    if enabled_tools is not None:
...
class MCPTool(BaseTool):
    """A docugraph BaseTool wrapper around a single MCP server tool.
