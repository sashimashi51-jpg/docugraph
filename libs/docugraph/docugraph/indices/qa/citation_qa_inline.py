...
from docugraph.base import AIMessage, Document, HumanMessage, SystemMessage
from docugraph.llms import PromptTemplate
...
FINAL ANSWER
An alternative to semantic chunking is fixed-size chunking. This traditional method involves splitting documents into chunks of a predetermined or user-specified size, regardless of semantic content, which is computationally efficientã€1ã€‘. However, it may result in the fragmentation of semantically related content, thereby potentially degrading retrieval performanceã€1ã€‘ã€2ã€‘.
...
CITATION_PATTERN = r"citationã€(\d+)ã€‘"
...
class AnswerWithInlineCitation(AnswerWithContextPipeline):
    ...
    def replace_citation_with_link(self, answer: str):
        # Define the regex pattern to match ã€numberã€‘
        pattern = r"ã€\d+ã€‘"
        alternate_pattern = r"\[\d+\]"

        # Regular expression to match merged citations
        multi_pattern = r"ã€([\d,\s]+)ã€‘"
        ...
        def split_citations(match):
            ...
            return "".join(f"ã€{num.strip()}ã€‘" for num in numbers)
        ...
        for citation in matched_citations:
            ...
            answer = answer.replace(
                ...
                (
                    ...
                    f"id='mark-{citation_id}'>ã€{citation_id}ã€‘</a>"
                ),
            )
