...
Quick and easy AI components to build Docugraph
...
```shell
pip install docugraph@git+ssh://git@github.com/Cinnamon/docugraph.git
```
...
purposes only. Thus, it should not be used in the main source code (i.e. `docugraph/` and `tests/`), but can be used in `examples/`.
