...
from docugraph.agents.tools.mcp import discover_tools_info, format_tool_list
...
class MCPManagement(BasePage):
    ...
    def on_register_events(self):
        # Add new server â€” save first, then fetch tools async
        self.btn_new.click(
            self.create_server,
            inputs=[self.config],
        ...
        # Save edits â€” save first, then refresh tools
    ...
    def _fetch_tools_markdown(self, config: dict) -> str:
        ...
        try:
            ...
            return f"âŒ Failed to fetch tools: {e}"
    ...
    def fetch_tools_for_add(self, config_str):
        ...
        try:
            ...
            return "âŒ Invalid JSON config"

        if not isinstance(configs, dict) or MCP_SERVERS_KEY not in configs:
            return f"âŒ Config must be a dictionary with '{MCP_SERVERS_KEY}' root key"

        mcp_servers = configs[MCP_SERVERS_KEY]
        if not isinstance(mcp_servers, dict):
            return f"âŒ '{MCP_SERVERS_KEY}' must be a dictionary"
